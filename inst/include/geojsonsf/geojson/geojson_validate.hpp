...
namespace validate {
	...
	inline void geojson_object_error(std::string key) {
		std::string err = "Invalid " + key + " object";
		Rcpp::stop(err);
	}

	inline void geojson_object_error(std::string key, int sfg_number) {
  ...
  inline void safe_parse_stream(Document& d, FileReadStream is ) {
	...
	inline void validate_points(const Value& v) {
		if ( v.Size() < 2 || v.Size() > 4 ) {
			geojson_object_error("lon/lat");
		}
	}

	inline void validate_point(const Value& v) {
		// TODO(move to header):
