...
Rcpp::NumericVector parse_point(const Value& coord_array, Rcpp::NumericVector& bbox) {
	Rcpp::NumericVector point = parse_point(coord_array);
  calculate_bbox(bbox, point);
  return point;
}

Rcpp::NumericVector get_point(const Value& point_array) {
	Rcpp::NumericVector point = parse_point(point_array);
	return point;
}
...
Rcpp::NumericMatrix parse_line(const Value& coord_array) {
	...
	for (int i = 0; i < n; i++) {
		line_string(i, _) = parse_point(coord_array[i]);
	}
	return line_string;
}
...
Rcpp::NumericMatrix get_multi_point(const Value& multi_point_array) {
	Rcpp::NumericMatrix multi_point = parse_line(multi_point_array);
	return multi_point;
}
...
Rcpp::NumericMatrix get_line_string(const Value& line_array) {
	Rcpp::NumericMatrix line_string = parse_line(line_array);
	return line_string;
}
...
Rcpp::List get_multi_line_string(const Value& multi_line_array) {
	...
	for (int i = 0; i < n; i++) {
		multi_line[i] = parse_line(multi_line_array[i]);
	}
	return multi_line;
}
...
Rcpp::List get_polygon(const Value& polygon_array) {
	...
	for (int i = 0; i < n; i++) {
		polygon[i] = parse_line(polygon_array[i]);
	}
	return polygon;
}
...
Rcpp::List get_multi_polygon(const Value& multi_polygon_array) {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < np; j++) {
			polygon[j] = parse_line(polygon_array[j]);
		}
