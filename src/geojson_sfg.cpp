...
Rcpp::List parse_geometry_collection_object(const Value& val, Rcpp::NumericVector& bbox) {
  ...
  for (int i = 0; i < val.Size(); i++) {
    ...
    parse_geometry_object(geom_collection, i, gcval, bbox);
  }
...
void parse_geojson(Document& d, const Value& v, Rcpp::List& sfc, int i,
  ...
  if (geom_type == "Feature") {
  	...
  	parse_geometry_object(sfc, i, geom, bbox);
...
Rcpp::List geojson_to_sf(const char* geojson, Rcpp::NumericVector& bbox) {
	...
	if (d.IsObject() ) {
		parse_geojson_object(d, sfg, bbox);
		return sfg;
	} else if (d.IsArray()) {

		Rcpp::List sfc(d.Size());
		for (int i = 0; i < d.Size(); i++) {
			parse_geojson_array(d, sfc, i, bbox);
		}
		return sfc;
	}
	return sfg;
...
Rcpp::List rcpp_geojson_to_sf(Rcpp::StringVector geojson) {
  ...
  //Rcpp::StringVector sfc_classes = start_sfc_classes(collectionCount);
  ...
  for (int i = 0; i < n; i++ ){
    lst[i] = geojson_to_sf(geojson[i], bbox);
  }
  return lst;
}
